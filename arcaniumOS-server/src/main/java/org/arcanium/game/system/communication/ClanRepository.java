...
import org.arcanium.net.packet.server.UpdateClanChat;
...
public final class ClanRepository {
    ...
    public boolean enter(Player player) {
        if (!owner.equals("arcanium") && players.size() >= MAX_MEMBERS) {
    ...
    public static ClanRepository get(String owner, boolean create) {
        ...
        if (details == null) {
            File file = new File(ServerConstants.PLAYER_DETAILS_PATH + owner + ".arcanium");
            if (!file.exists()) {
                return null;
            }
