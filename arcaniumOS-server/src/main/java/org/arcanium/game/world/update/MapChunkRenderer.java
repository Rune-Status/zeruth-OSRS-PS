        ...
        for (int x = 0; x < last.length; x++) {
            ...
            for (int y = 0; y < chunks.length; y++) {
                ...
                if (!containsChunk(v.getChunks(), previous)) {
                    PacketRepository.send(UpdateZonePartialFollows.class, new AreaPositionContext(player, player.getLocation(), player.getLocation().getChunkOffsetX(), player.getLocation().getChunkOffsetY()));
                } else {
