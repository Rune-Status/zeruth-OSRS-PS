...
import org.arcanium.cache.def.impl.ConfigFileDefinition;
...
import org.arcanium.game.node.entity.impl.GameAttributes;
...
public class GameObject extends Node {
	...
	if (def.getConfigFileId() > -1) {
		...
		player.getConfigManager().sendVarp(config.getConfigId(), value);
	    }
	} else if (def.getConfigId() > -1) {
	    player.getConfigManager().sendVarp(def.getConfigId(), index);
	}
    }
