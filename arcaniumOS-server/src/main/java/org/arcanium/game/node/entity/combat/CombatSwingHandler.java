...
public abstract class CombatSwingHandler {
    ...
    public boolean isAccurateImpact(Entity entity, Entity victim, CombatStyle style, double accuracyMod, double defenceMod) {
        ...
        if (ServerConstants.DEBUG && entity.isPlayer() && entity.asPlayer().getDetails().getUsername().equalsIgnoreCase("trees")) {
            ((Player) entity).getPacketDispatch().sendMessage("[Combat Swing] Your Accuracy=" + acc + " Opponent Defence=" + def);
        }
