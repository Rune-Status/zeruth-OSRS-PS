package org.arcanium.game.node.entity.npc;

import org.arcanium.cache.def.impl.NPCDefinition;
import org.arcanium.game.content.global.shop.Shop;
import org.arcanium.game.content.skill.Skills;
import org.arcanium.game.content.skill.p2p.slayer.Task;
import org.arcanium.game.content.skill.p2p.slayer.Tasks;
...
import org.arcanium.game.node.entity.combat.equipment.DefaultCombatSpell;
...
import org.arcanium.game.system.mysql.impl.ShopSQLHandler;
...
import org.arcanium.game.world.update.flag.npc.NPCFaceEntity;
import org.arcanium.game.world.update.flag.npc.NPCFacePosition;
import org.arcanium.game.world.update.flag.npc.NPCForceChat;
import org.arcanium.game.world.update.flag.npc.NPCTransform;
...
public class NPC extends Entity {
    ...
    public void initConfig() {
        ...
        getProperties().setBonuses(definition.getConfiguration(NPCConfigSQLHandler.BONUSES, new int[16]));
    ...
    public void finalizeDeath(Entity killer) {
        ...
        if (task != null && killer instanceof Player && ((Player) killer).getSlayer().getTask() == task) {
            killer.getSkills().addExperience(Skills.SLAYER, getDefinition().getConfiguration(NPCConfigSQLHandler.SLAYER_EXP, (double) getSkills().getStaticLevel(Skills.HITPOINTS)), true);
    ...
    public boolean faceLocation(Location location) {
        ...
        getUpdateMasks().register(new NPCFacePosition(location));
        return true;
    }
    ...
    public void transform(int id) {
        ...
        initConfig();
        interaction.setDefault();
        getUpdateMasks().register(new NPCTransform(id));
    }
    ...
    protected Location getMovementDestination() {
        if (!pathBoundMovement || movementPath == null || movementPath.length < 1) {
            return getProperties().getSpawnLocation().transform(-1 + RandomFunction.random(getWalkRadius()), -1 + RandomFunction.random(getWalkRadius()), 0);
        }
