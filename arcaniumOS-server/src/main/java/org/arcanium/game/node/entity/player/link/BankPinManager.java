...
public class BankPinManager implements SavingModule {
    ...
    private void shuffleDigits() {
        ...
        player.getConfigManager().sendVarp(562, bitValue);
        player.getConfigManager().sendVarp(563, digits.get(8) | digits.get(9) << 4 | stage << 26);
        for (int i = 0; i < 9; i++) {
    ...
    public void drawLoginMessage() {
        player.getPacketDispatch().sendString(status.getLoginMessage(this), 378, 17);
        checkPendingActivity();
    }
