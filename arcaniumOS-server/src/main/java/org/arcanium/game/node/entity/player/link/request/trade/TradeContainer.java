...
import org.arcanium.game.content.global.quest.impl.free.ShieldofArrav;
...
public final class TradeContainer extends Container {
	...
	public void update(Container c, ContainerEvent event) {
	    PacketRepository.send(PartialContainerPacket.class, new ContainerContext(player, -1, 2, 24, c.toArray(), c.capacity(), false));
	    if (TradeModule.getExtension(player) != null && TradeModule.getExtension(TradeModule.getExtension(player).getTarget()) != null) {
		PacketRepository.send(PartialContainerPacket.class, new ContainerContext(player, -1, 2, 23, TradeModule.getExtension(TradeModule.getExtension(player).getTarget()).getContainer().toArray(), 27, false));
	    }
	    checkAlert();
	}
	...
	public void refresh(Container c) {
	    PacketRepository.send(PartialContainerPacket.class, new ContainerContext(player, -1, 2, 93, player.getInventory(), false));
	}
