package org.arcanium.game.content.global.travel.canoe;

import org.arcanium.game.component.Component;
import org.arcanium.game.content.skill.Skills;
import org.arcanium.game.content.skill.f2p.gather.SkillingTool;
...
public final class CanoeExtension {
	...
	player.getConfigManager().sendVarp(675, (station.ordinal() + 1) << 17);
	GameWorld.submit(new Pulse(4, player) {
	    @Override
	    public boolean pulse() {
		player.animate(new Animation(-1, Animator.Priority.HIGH));
		player.getConfigManager().sendVarp(674, CONFIGS[0] << (station.ordinal() * 8));
		player.getConfigManager().sendVarp(674, CONFIGS[1] << (station.ordinal() * 8));
	...
	player.getPulseManager().run(new Pulse(3) {
		...
		if (RandomFunction.random(canoe == Canoe.WAKA ? 8 : 6) == 1) {
		    ...
		    player.getConfigManager().sendVarp(674, station == CanoeStation.BARBARIAN_VILLAGE ? station.getCraftConfig(canoe, false) : (CONFIGS[2] << (station.ordinal() * 8)) + station.getCraftConfig(canoe, false));
		    player.getConfigManager().sendVarp(675, (station.ordinal() + 1) << 17);
	...
	player.animate(PUSH);
	player.getConfigManager().sendVarp(674, (CONFIGS[3] << (station.ordinal() * 8)) + station.getCraftConfig(canoe, true));
	GameWorld.submit(new Pulse(1) {
		...
		if (counter == 1) {
		    ...
		    player.getConfigManager().sendVarp(674, (CONFIGS[4] << (station.ordinal() * 8)) + station.getFloatConfig(canoe));
		    setStage(3);
		}
		counter += 1;
		return false;
	...
	GameWorld.submit(new Pulse(1) {
		...
		case 17:
		    ...
		    setStage(0);
		    player.unlock();
		    player.getConfigManager().sendVarp(674, 0);
		    return true;
		}
		return false;
	    }
