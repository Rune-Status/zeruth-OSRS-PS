...
import org.arcanium.game.world.map.zone.ZoneRestriction;
 ...
 * @author Vexia
 */
public abstract class HolidayEvent extends MapZone {
	...
	if (player.getDetails().getRights() == Rights.ADMINISTRATOR) {
	    switch (name) {
	    case "reset":
		player.getEmotes().lock(EmoteData.ZOMBIE_HAND);
		setStageValue(player, 0);
		player.sendMessage("Reset event.");
		return true;
	    case "stage":
		player.sendMessage("Stage = " + getStage(player));
		return true;
	    case "setstage":
		player.getEmotes().lock(EmoteData.ZOMBIE_HAND);
		setStage(player, Integer.parseInt(arguments[1]));
		player.sendMessage("Set stage to " + arguments[1]);
		return true;
	    }
	}
	return false;
