...
import org.arcanium.net.packet.server.GrandExchangePacket;
...
import static org.arcanium.game.node.entity.player.link.CS2Repository.COLLECTION_BOX_SCRIPT;
...
public final class GrandExchange implements SavingModule {
    ...
    public void open() {
     ...
     /*   player.getInterfaceManager().open(new Component(465)).setCloseEvent(new CloseEvent() {
            @Override
    ...
    public void openCollectionBox() {
        ...
        player.getPacketDispatch().sendCS2Script(COLLECTION_BOX_SCRIPT, new Object[]{-1, -1});//ii
        player.getPacketDispatch().sendInterface(player.getInterfaceManager().getWindowsPane(), 21, 402, false);
    ...
    public void toMainInterface() {
        ...
        player.getConfigManager().sendVarp(563, 0);
        player.getConfigManager().sendVarp(375, 0);
        player.getConfigManager().sendVarp(1151, -1);
        player.getPacketDispatch().sendCS2Script(828, new Object[]{player.isDonator() ? 1 : 0}); // Slot Count (Members or Non Members)
        player.getPacketDispatch().sendCS2Script(917, new Object[]{-1, -1});//ii
        player.getPacketDispatch().sendInterface(player.getInterfaceManager().getWindowsPane(), 21, 465, false);
        player.getPacketDispatch().sendInterface(164, 1, 467, true);
        player.getPacketDispatch().sendCS2Script(915, new Object[]{3});//i
        player.getPacketDispatch().sendString("Click the icon on the left to search for items.", 465, 25);
        player.getPacketDispatch().sendString("", 465, 26);
        player.getPacketDispatch().sendString("", 465, 16);
        player.getPacketDispatch().sendString("", 465, 17);
        player.getPacketDispatch().sendCS2Script(915, new Object[]{3});//i
    ...
    public void openBuy(int index) {
        if (player.getDetails().getRights() == Rights.ADMINISTRATOR && (!player.getName().equals("vexia") && !player.getName().equals("splinter") && !player.getName().equals("emperor"))) {
            player.sendMessage("You can't open that as an Arcanium moderator.");
            return;
        }
