...
import org.arcanium.game.world.map.Point;
import org.arcanium.game.world.map.path.Path;
...
public abstract class MovementPulse extends Pulse {
    ...
    public void findPath() {
        ...
        if (ServerConstants.DEBUG) {
            if (mover.isPlayer()) {
                System.out.println("[Player Movement Pulse] Path is " + (path == null ? "null" : "not null") + " : 'near' var was " + (near ? "unsuccessful" : "successful"));//+ " handler=" + optionHandler.getClass().toGenericString());
            }
        }
        if (!path.getPoints().isEmpty()) {
            ...
            while (!path.getPoints().isEmpty()) {
                ...
                if (ServerConstants.DEBUG) {
                    if (mover.isPlayer()) {
                        System.out.println("[Player Movement Pulse] Adding path points");
                    }
                }
        ...
        if (destination instanceof Entity) {
            ...
            if (ServerConstants.DEBUG) {
                if (mover.isPlayer()) {
                    System.out.println("[Player Movement Pulse] Facing entity at " + destination.getLocation().toString());
                }
            }
        } else {
            if (ServerConstants.DEBUG) {
                if (mover.isPlayer()) {
                    System.out.println("[Player Movement Pulse] Facing null");
                }
            }
            mover.face(null);
        }
