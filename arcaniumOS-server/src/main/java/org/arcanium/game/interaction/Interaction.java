package org.arcanium.game.interaction;
...
import org.arcanium.net.packet.server.InteractionOption;
...
public final class Interaction {
    ...
    public void handle(final Player player, final Option option) {
        ...
        if (!hasHandler || walk) {
            System.out.println("Handling walk option interaction");
            handleWalkOption(player, option, pulseType);
        } else if (hasHandler) {
            System.out.println("Option handler being used=" + option.getHandler().getClass().getSimpleName());
            handleDefaultOption(player, option, pulseType);
        } else {
            System.out.println("Handling unhandled option interaction");
    ...
    public void handleItemOption(final Player player, final Option option, final Container container) {
        ...
        System.out.println("Handling Item Option " + option.getName());
        if (option.getHandler() != null) {
            option.getHandler().handle(player, node, option.getName().toLowerCase());
            return;
        }
    ...
    private void handleWalkOption(final Player player, final Option option, String pulseType) {
        if (node.getLocation() == null) {
            if (ServerConstants.DEBUG) {
                System.out.println("[Interaction Walk Option] node location is invalid... running unhandled action");
            }
            player.getPulseManager().runUnhandledAction(player, pulseType);
            return;
        }
    ...
    private void handleDefaultOption(final Player player, final Option option, String pulseType) {
        ...
        player.getPulseManager().run(new Pulse(1, player, node) {
            ...
            public boolean pulse() {
                ...
                if (ServerConstants.DEBUG) {
                    player.sendMessage("Default Option :" + option.toString() + " | " + pulseType);
                }
                if (!option.getHandler().handle(player, node, option.getName().toLowerCase())) {
                    player.getPacketDispatch().sendMessage("Nothing interesting happens.");
                }
    ...
    public void init(int nodeId, String... names) {
        ...
        for (int i = 0; i < options.length; i++) {
            ...
            if (name != null && !name.equals("null")) {
                //System.out.println("[Interaction] Initializing option " + name + " / " + nodeId + " / " + node.getName());
