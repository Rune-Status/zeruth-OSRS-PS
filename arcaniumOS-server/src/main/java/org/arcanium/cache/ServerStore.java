...
public final class ServerStore {
    ...
    public static void init(String path) {
        storage = new HashMap<>();
        File file = new File(path + "/static_cache.arcanium");
        if (file.exists()) {
        ...
        file = new File(path + "/dynamic_cache.arcanium");
        if (file.exists()) {
            try (RandomAccessFile raf = new RandomAccessFile(file, "rw")) {
                FileChannel channel = raf.getChannel();
                ByteBuffer buffer = channel.map(MapMode.READ_WRITE, 0, channel.size());
    ...
    public static void createStaticStore(String path) {
        write(path + "/static_cache.arcanium", false);
    }
    ...
    public static void dump(String path) {
        write(path + "/dynamic_cache.arcanium", true);
    }
