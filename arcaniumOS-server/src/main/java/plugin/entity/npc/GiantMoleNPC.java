...
import org.arcanium.game.content.global.BossKillCounter;
import org.arcanium.game.content.global.LightSource;
import org.arcanium.game.content.global.action.DigAction;
import org.arcanium.game.content.global.action.DigSpadeHandler;
...
import org.arcanium.game.world.map.zone.impl.DarkZone;
...
public final class GiantMoleNPC extends AbstractNPC {
     ...
     */
    private static final Animation DIG_ANIMATION = new Animation(3314, Animator.Priority.VERY_HIGH);
     ...
     */
    private static final Animation DIG_UP_ANIMATION = new Animation(3315, Animator.Priority.VERY_HIGH);
    ...
    private void splatterMud(Location hole) {
        for (Player p : RegionManager.getLocalPlayers(getCenterLocation(), (size() >> 1) + 2)) {
            PacketRepository.send(Interface.class, new InterfaceContext(p, p.getInterfaceManager().getWindowsPane(), 77, 226, true));
